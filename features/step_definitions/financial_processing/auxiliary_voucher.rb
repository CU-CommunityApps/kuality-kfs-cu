...
And /^I add credit and debit accounting lines with two different sub funds$/ do
  ...
  while (split_code_description_at_first_hyphen(target_account['subFundGroup.codeAndDescription'][0])).eql?(split_code_description_at_first_hyphen(source_account['subFundGroup.codeAndDescription'][0]))
    target_account = get_random_account
  end
